...
emperor_killers %>% 
  mutate(
    killer_type = ifelse(killer == "Praetorian Guard" | killer == "Own Army" , TRUE, FALSE),
    # use `fct_inorder()` to maintain the way we sorted the data
    killer = fct_inorder(killer)
  ) %>%
  ggplot(aes(x = n, y = killer, fill = killer_type)) +
  geom_col() +
  ...
  labs(colour = "killer_type",
       title = "<b>categories of Roman emperor killers</b><br>
       <span style = 'font-size:12pt'>Many emperors were murdered by their <span style='color:#FF6347'>own trusted soldiers</span>.</span>",
       x= "Number of deaths",
       y = "")
